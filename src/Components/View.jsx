...
const View = () => {
    ...
    return (
        <>
            <div className='shadow-2xl bg-black' style={{  backgroundImage: 'linear-gradient(to right,#38CBFB  , #E83283)',width:'113.3%',marginLeft:'-100px',} }>
                <Create onNoteAdded={fetchCollectionData} />
    
                <div className='flex flex-col justify-center items-center mx-40 my-4 ' >
                    <h1 className='text-5xl font-bold mt-16 text-white'> ADD YOUR NOTES</h1>
                    <div className="grid lg:grid-cols-4 md:grid-cols-2 sm:grid-cols-1 grid-flow-row gap-4 mt-24 mb-36">
                        {notes.length > 0 ? (
                            notes.map(note => (
                                <div key={note.id} className='rounded-md border shadow p-3 bg-black w-52 h-40'>
                                    <div className='flex justify-between items-center mb-2'>
                                        <h1 className='font-bold uppercase text-white'>{note.title || "Title"}</h1>
                                        <div className='w-12 flex justify-between'>
                                            <button onClick={() => handleOpen(note)}><i className="fa-solid fa-file-pen text-indigo-600"></i></button>
                                            <button onClick={() => handleDelete(note.id)}><i className="fa-solid fa-trash text-red-400"></i></button>
                                        </div>
                                    </div>
                ...
                >
                    <Box sx={style}>
                        ...
                        <button className='mt-5 bg-black rounded px-5 py-1 text-lg font-bold text-white' onClick={handleNoteSave}>Save</button>
                    </Box>
                </Modal>
            </div>
        </>
